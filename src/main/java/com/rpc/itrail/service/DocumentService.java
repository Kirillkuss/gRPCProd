...
import com.rpc.itrail.DocumentServiceGrpc;
...
public class DocumentService extends DocumentServiceGrpc.DocumentServiceImplBase {
    ...
    public void addDocument( DocumentResponse request , StreamObserver<DocumentResponse> response ){
        try{
            if  ( documentRepository.findById( request.getIdDocument() ).isPresent() ) throw new Exception( "Документ с таким ИД уже существует");
            if ( documentRepository.findByNumar( request.getNumar()).isPresent() ) throw new Exception( "Документ с таким номером документа уже существует");
            if ( documentRepository.findByPolis( request.getPolis()).isPresent() ) throw new Exception( "Документ с таким полисом уже существует");
            if ( documentRepository.findBySnils( request.getSnils()).isPresent() ) throw new Exception( "Документ с таким СНИЛСом уже существует");
